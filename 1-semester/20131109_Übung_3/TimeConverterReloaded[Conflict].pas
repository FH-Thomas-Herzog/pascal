 ...
 
 {
  ...
  BEGIN
    ...
    IF HasErrors(span) THEN BEGIN
      ...
      IF span.actionType = TIME_TO_SECOND THEN BEGIN
      ...
      ELSE IF (span.actionType = SECOND_TO_TIME) AND (span.errorArray[SECOND] = INVALID_SECONDS) THEN
        errors := errors + '[' + ToString(span.timeInSeconds) + ']'
      {
        Get the error for 'CALC_DIFFERENCE' 
      }
      ELSE IF (span.actionType = CALC_DIFFERENCE) AND (span.errorArray[OVERFLOW] = SECOND_OVERFLOW) THEN
  ...
  BEGIN
    ...
    IF (HasErrors(result)) AND (HasErrors(beforeResult)) AND (HasErrors(afterResult)) THEN BEGIN
      WriteLn(HandleErrors(result))
    END
    ELSE 
...
BEGIN
  ...
  WriteLn('########################################################################');
  WriteLn();
  Test(InitTimeSpan(0,0,0,100,CALC_DIFFERENCE), InitTimeSpan(0,0,0,99,CALC_DIFFERENCE));
  Test(InitTimeSpan(0,0,0,100,CALC_DIFFERENCE), InitTimeSpan(0,0,0,100,CALC_DIFFERENCE));
  Test(InitTimeSpan(0,0,0,100,CALC_DIFFERENCE), InitTimeSpan(0,0,0,101,CALC_DIFFERENCE));
  Test(InitTimeSpan(0,0,0,100,CALC_DIFFERENCE), InitTimeSpan(0,0,0,158,CALC_DIFFERENCE));
