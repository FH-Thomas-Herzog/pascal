{
  ...
  open addressing.
  Here the node is inserted on a free index after the already used index as long
  as the hash table contains free indexes after the used one.
  If the end of the hash tbale is reached and no free index could be found
  then this implementation is not able to insert the element.
}
...

{
  ...
  thia method searchs for the next avialable index in the has table.
  If the end of the has table is reached this methods sops and
  the given node will not be added to the hash table.
...
Begin
  if node <> nil then begin
    ...
    if ht[hash] = nil then begin
      ...
      nodeContainer := ht[hash];
      while (hash < (High(THashTableIndex) - Low(THashTableIndex) - 1)) and (nodeContainer <> nil) do begin
        hash := hash +1;
        nodeContainer := ht[hash];
      end;
      if (hash < (High(THashTableIndex) - Low(THashTableIndex) - 1)) then begin
        ht[hash] := node;
      end;
    end;
  end;
...
Begin
  ...
  node := ht[hash];
  while (hash < (High(THashTableIndex) - Low(THashTableIndex) - 1)) and (node <> nil) and (node^.data <> value) do begin
    hash := hash +1;
    node := ht[hash];
  end;
  FindValue := (ht[hash] <> nil) and (ht[hash]^.data = value);
End;
