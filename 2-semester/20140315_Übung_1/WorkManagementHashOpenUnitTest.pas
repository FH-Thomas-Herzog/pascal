...
Begin
  ...
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 0, 60), TimeSpanUnit.CreateTimeSpan(0, 2, 0), 10, state);
  writeln;
  
  // Invalid MaximumSpan
  PrintUtils.PrintHeader('Invalid MaximumSpan');
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 0, 1), TimeSpanUnit.CreateTimeSpan(0, 0, 60), 10, state);
  writeln;
  
  // Invalid Range
  PrintUtils.PrintHeader('Invalid Range');
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 0, 30), TimeSpanUnit.CreateTimeSpan(0, 0, 10), 10, state);
  writeln;
  
  // Invalid StorageSize
  PrintUtils.PrintHeader('Invalid Storage size');
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 0, 30), TimeSpanUnit.CreateTimeSpan(0, 0, 50), 0, state);
  writeln;
  
  // Valid context
  PrintUtils.PrintHeader('Valid context definition');
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 0, 30), TimeSpanUnit.CreateTimeSpan(0, 0, 50), 10, state); 
End;
...
Var
  ...
  state: StateCode;
Begin
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 1, 0), TimeSpanUnit.CreateTimeSpan(8, 0, 0), 100, state); 
  // Invalid TimeSpan
...
Begin
  ...
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 1, 0), TimeSpanUnit.CreateTimeSpan(8, 0, 0), 4, state);
  span := TimeSpanUnit.CreateTimeSpan(8, 0, 0);
  entry := WorkManagementHashOpenUnit.CreateWorkEntry(span, state);
  ...
  for i := 1 to 3 do begin
    ...
    writeln('Set spendTime : ', TimeSpanUnit.TimeSpanToString(span));
    state := WorkManagementHashOpenUnit.AddWorkEntry('thomas_' + IntToStr(i), entry);
    writeln('State         : ', state); 
    writeln('-------------------------------------');
  ...
  // Normal behavior
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 1, 0), TimeSpanUnit.CreateTimeSpan(8, 0, 0), 3, state);
...
Var
  ...
  i, j: Integer;
Begin 
  WorkManagementHashOpenUnit.SetWorkEntryContext(TimeSpanUnit.CreateTimeSpan(0, 1, 0), TimeSpanUnit.CreateTimeSpan(8, 0, 0), 10, state);
  ...
  WorkManagementHashOpenUnit.GetTotalWorkTimeForPerson('thomas_11', span, state);
  ...
  WorkManagementHashOpenUnit.GetTotalWorkTimeForPerson('thomas_10', span, state);
...
Begin 
  ...
  for i := 1 to 5 do begin
    For j := 1 to 5 do begin
      span := TimeSpanUnit.CreateTimeSpan(0, 1, j);
      entry := WorkManagementHashOpenUnit.CreateWorkEntry(span, state);
      WorkManagementHashOpenUnit.AddWorkEntry('thomas_' + IntToStr(i), entry);
      WorkManagementHashOpenUnit.AddWorkEntry('wolfganf_' + IntToStr(i), entry);
    end;
  end; 
  
  // Person not found
  PrintUtils.PrintHeader('Person not found in storage');
  state := WorkManagementHashOpenUnit.RemovePerson('thomas_6');
  ...
  state := WorkManagementHashOpenUnit.RemovePerson('thomas_4');
  writeln('Person: ', 'thomas_4');  
  writeln('State : ', state); 
  state := WorkManagementHashOpenUnit.RemovePerson('wolfganf_5');
